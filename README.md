...
| `--align_home_probes` | `0` | `align` with `exact-hash`/`kmer` only: map each distinct amplicon error-free and this many times one base off through MAPseq (same database and `--mapseq_args` as the reads), and put its distance-0 mass where those land — `(1-e)**L` on the error-free label, the rest as the probes did. Replaces the assumption that a read from a group is labelled with that group, which fails for 12% of SILVA NR99 V4 groups even with AAP's flags. `20` costs 5.8M MAPseq queries on SILVA (34 min, 12 threads) and needs the set's `.mscluster`. See [the note](dev/mapseq_candidate_funnel.md). |
